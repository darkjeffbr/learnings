...
/16|last two numbers can change
...
## NAT Instances - Network Address Translation (outdated but still in at the exam)
...
- **enableDnsHostname:** (=DNS Hostname setting)
	- False by default on newly created VPC, True by default for Default VPC
...
- Default NACL allows everything outbound and everything inboud
- **One NACL per Subnet, new Subnets are assigned the Default NACL**
- Define NACL rules:
	- Rules have a number (1-32766) and higher precedence with a lower number
		- Ex.: If you define #100 ALLOW <IP> and #200 DENY <IP>, IP will be allowed
	- Last rule is an asterisk and denies a request in case of no rule match
	- AWS recommends adding rules by increment of 100
- Newly created NACL will deny everything
- NACL are great way of blocking a specific IP at the subnet level

### Network ACL with Ephemeral Ports
...
- VPC Peering connection is **not transtive** (must be established for each VPC that need to communicate with one another)
...
- Capture network information from AWS managed interfaces too: ELB, RDS, ElastiCache, Redshift, WorkSpaces
...
- Examp Tip: Make sure the bastion host only has port 22 traffic from the IP you need, not from the security groups of your other instances
...
- Customer Gateway:
	...
	- IP Address:
		- Use static, internet-routable IP address for your custome gateway device
...
## Transit Gateway
- For having transitive peering between thousands of VPC and on-premises, hub-and-spoke (start) connection
